...
foreach($partie->getmanche() as $manche){ // loop on rounds (manches)...
  foreach($manche->gettours() as $tour){ // then loop on turns (tours)...
    foreach ($tour->getresultat() as $resultat){ // then loop on results (resultats)...
      ...
      if ($resultat->getid_joueur() == 999){ // if property id_joueur of current result is 999 (means it is the correct definition, not a real player)...
        ...
        foreach ($resultat->getid_vote() as $vote){
